...
router.post('/Signup', (req, res, next) => {
  ...
  .then((user) => {
    if(!user){
      hashpassword = bcrypt.hash(req.body.password, 8);
  ...
  if(!req.cookies.auth)
  {
    ...
    .then((user) => {
      if(!user){
        ...
        res.statusCode = 403;
        res.setHeader('Content-Type', 'application/json');
        res.json({success: false, status: "Password is not corrent!"});
      }
      else if(user.username == req.body.username && bcrypt.compare(user.password, req.body.password)){
